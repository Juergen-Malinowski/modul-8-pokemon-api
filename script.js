...
async function loadPokemon() {
  try {
    ...
    if (startIndex >= allPoke.length) {
      ...
      const pokemonList = await fetchJson(
        `${POKE_API_BASE_URL}?limit=${limit}&offset=${startIndex - 1}`,
      );
    ...
    document.getElementById("overview_poke").textContent =
      "Pokémon data could not be loaded. Please try again.";
    if (allPoke.length > 0) {
      renderControlPanel();
    }
    return false;
...
function showPokemon() {
  ...
  for (
    ...
    index < endIndex && index < allPoke.length;
    index++
  ) {
    arrayID = index;
    findBackgroundColor();
...
window.addEventListener("orientationchange", () => {
  document
    .getElementById("rotate_to_portrait_hint")
    .classList.remove("show_orientation_hint");
});
